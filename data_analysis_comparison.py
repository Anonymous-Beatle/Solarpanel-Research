...
#-----PARAMETERS:-----
day_date = datetime(2023,2,13,0,0)
lat = 51.113911     #Hooiweg 31, 2222 Itegem
long = 4.709500
solar_panel_file_path = "solar_data_2023-02-13_raw.txt"         #dit nog aanpassen zodat de datum vanzelf wordt ingevuld
winteruur = True
...
fig = plt.figure(figsize=(11,6), layout="constrained")
fig.suptitle(f"Solar Panel Power at two different days", fontsize=15, fontweight= "bold")
ax1 = fig.add_subplot(1,2,1)

ax1.plot(times, power, '-', color='firebrick', label="Power")
ax1.set_title(day_date.strftime("%Y-%m-%d"))
for i in [4]: #[4,5,6]: #range(10):#[4,5,6]:
    ...
    print("Maximum in mei: ", max(attenuated_areas[i]))
    pred_max1 = max(attenuated_areas[i])    
    real_max1 = max(power)
    ax1.plot(sun_times, scaled_areas, '-', color="violet", label=f"{i*10} degrees, without attenuation")
...
ax2.plot(times, power, '-', color='firebrick', label="Power")
ax2.set_title(day_date.strftime("%Y-%m-%d"))

for i in [4]: #[4,5,6]: #range(10):#[4,5,6]:
    a = attenuated_areas[i]
    scale_factor = max(power)/max(a)
    scaled_areas = [area * scale_factor for area in a]
    #ax2.plot(sun_times, scaled_areas, '-', label=f"{i*10} degrees, with attenuation")
    
    b = areas_at_angles[i]
    scale_factor = max(power)/max(b)
    scaled_areas = [area * scale_factor for area in b]
    ax2.plot(sun_times, scaled_areas, '-', color="violet", label=f"{i*10} degrees, without attenuation")
    
    pred_max2 = max(attenuated_areas[i])    
    real_max2 = max(power)
...
ax2.set_ylabel("Power (kW)", fontsize=10, fontweight= "bold")
ax2.grid(True)
...
print(pred_max1, pred_max2)
print(real_max1, real_max2)

print("Fraction Prediction", pred_max1/pred_max2)
print("Fraction Real", real_max1/real_max2)

plt.savefig("Modelled_power_comparison_no_attenuation.png",dpi=600)
plt.show()